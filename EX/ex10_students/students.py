...
def filter_by_course(student_list: list, course: str) -> list:
    ...
    """
    return [name for name in student_list if course in name.courses]
...
def succeeding_students(student_list: list) -> list:
    ...
    """
    return [name for name in student_list if not is_failing(name)]
...
def failing_students(student_list: list) -> list:
    ...
    """
    return [name for name in student_list if is_failing(name)]
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    sorted_grade_list = sorted(student_list, key=lambda x: (x.grade * -1, x.name))
    return sorted_grade_list
...
def sort_by_worst_grade(student_list: list) -> list:
    ...
    for name in student_list:
        if is_failing(name):
            student_list.remove(name)
    sorted_grade_list = sorted(student_list, key=lambda x: (x.grade, x.name))
    return sorted_grade_list
