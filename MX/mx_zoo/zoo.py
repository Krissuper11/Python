...
def find_smallest_animal_by_weight(animal_list: list) -> Animal:
    ...
    """
    return min(animal_list, key=lambda x: x.weight_range)
...
def find_how_many_pumpkins_are_needed_to_feed_animals(animal_list: list) -> int:
    ...
    """
    return functools.reduce(lambda x, y: round(sum(x.weight_range) * 0.06, 1) if not isinstance(x, float)
    else x + math.ceil(sum(y.weight_range) * 0.06), list(filter(lambda x: x.diet != "carnivorous", animal_list))) / 3 * 90
...
def create_animal_descriptions(animal_list: list) -> list:
    ...
    return list(map(lambda x: f"{x.species} ({x.scientific_name}) lives in {x.habitat} and its diet is {x.diet}."
                              f" These animals can live up to {x.age_up_to} years"
                              f" and they weigh between {x.weight_range[0]} kg and {x.weight_range[1]} kg as adults.",
                    animal_list))
