...
def close_far(a: int, b: int, c: int) -> bool:
    ...
    num2 = a - c
    num3 = b - c
    if -1 <= num1 <= 1 and not -1 <= num2 <= 1 and not -1 <= num3 <= 1:
        return True
    if not -1 <= num1 <= 1 and -1 <= num2 <= 1 and not -1 <= num3 <= 1:
        return True
    return False
...
def create_student(name: str, grades: list, credit_points: int) -> Student:
    ...
    """
    average_grade = round(sum(grades) / len(list), 3)
...
def get_top_student_with_credit_points(students: list, min_credit_points: int):
    ...
    """
    students_by_points = sorted(students, key=lambda x: x.credit_points, reverse=True)
